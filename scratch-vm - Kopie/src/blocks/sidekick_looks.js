...
class SidekickLooksBlocks {
  constructor(runtime) {
    ...
    this.runtime.on(SidekickLooksBlocks.SAY_OR_THINK, this._updateBubble);
  }
  ...
  _getBubbleState(target) {
    let bubbleState = target.getCustomState(SidekickLooksBlocks.STATE_KEY);
    if (!bubbleState) {
      bubbleState = Clone.simple(SidekickLooksBlocks.DEFAULT_BUBBLE_STATE);
      target.setCustomState(SidekickLooksBlocks.STATE_KEY, bubbleState);
    }
    return bubbleState;
  }
  ...
  _formatBubbleText(text) {
    ...
    // text = String(text).substr(0, SidekickLooksBlocks.SAY_BUBBLE_LIMIT);

    // Replacing substr() with substring() since "The declaration was marked as deprecated here".
    // The main advantage of substr() seems to be to be able to specify a negative start position
    // (information from: https://stackoverflow.com/questions/52640271/why-is-string-prototype-substr-deprecated).
    // Since the start position of "0" is specified, I think it can simply be replaced with substring() here,
    // However, we have to add 1 to the second parameter of substring() to here achieve the same functionality as with substr(),
    // since the second parameter of substring() "end" is a Zero-based index number indicating the end of the substring
    // (instead of the the second parameter "length" of substr(), setting the number of characters to include in the returned substring).
    text = String(text).substring(0, SidekickLooksBlocks.SAY_BUBBLE_LIMIT + 1);
  ...
  say(args, util) {
    ...
    this.runtime.emit(
      SidekickLooksBlocks.SAY_OR_THINK,
      util.target,
      "say",
      args.MESSAGE
    );
  ...
  _setCostume(target, requestedCostume, optZeroIndex) {
    if (typeof requestedCostume === "number") {
      ...
      if (costumeIndex !== -1) {
        ...
        !(isNaN(requestedCostume) || Cast.isWhiteSpace(requestedCostume))
      ) {
        target.setCostume(
  ...
  _setBackdrop(stage, requestedBackdrop, optZeroIndex) {
    if (typeof requestedBackdrop === "number") {
      ...
      if (costumeIndex !== -1) {
        ...
        !(isNaN(requestedBackdrop) || Cast.isWhiteSpace(requestedBackdrop))
      ) {
        stage.setCostume(
          optZeroIndex
    ...
    switch (effect) {
      case "ghost":
        clampedValue = MathUtil.clamp(
          ...
          // ???
          // !!!
          SidekickLooksBlocks.EFFECT_GHOST_LIMIT.min,
          SidekickLooksBlocks.EFFECT_GHOST_LIMIT.max
        );
        break;
      case "brightness":
        clampedValue = MathUtil.clamp(
          value,
          SidekickLooksBlocks.EFFECT_BRIGHTNESS_LIMIT.min,
          SidekickLooksBlocks.EFFECT_BRIGHTNESS_LIMIT.max
        );
        break;
    }
    return clampedValue;
...
module.exports = SidekickLooksBlocks;
