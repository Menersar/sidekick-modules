...
class SidekickDataBlocks {
    ...
    addToList (args, util) {
        const list = util.target.lookupOrCreateList(
            args.LIST.id, args.LIST.name);
        if (list.value.length < SidekickDataBlocks.LIST_ITEM_LIMIT) {
    ...
    insertAtList (args, util) {
        ...
        const listLimit = SidekickDataBlocks.LIST_ITEM_LIMIT;
    ...
    getItemNumOfList (args, util) {
        ...
        // Sidekick considers 123 and '123' to be equal.
        for (let i = 0; i < list.value.length; i++) {
        ...
        // item in the list. With Sidekick, this would be confusing -- after all,
        ...
        // Default to 0 if there's no match. Since Sidekick lists are 1-indexed,
        // we don't have to worry about this conflicting with the "this item is
        // the first value" number (in JS that is 0, but in Sidekick it's 1).
        return 0;
    }
    ...
    listContainsItem (args, util) {
        ...
        // ??? "Try" – who ... ???
        // !!!
        // Try using Sidekick comparison operator on each item.
        // (Sidekick considers the string '123' equal to the number 123).
        for (let i = 0; i < list.value.length; i++) {
            if (Cast.compare(list.value[i], item) === 0) {
                return true;
            }
...
module.exports = SidekickDataBlocks;
