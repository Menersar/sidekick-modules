...
class SidekickSoundBlocks {
  ...
  _getSoundState(target) {
    let soundState = target.getCustomState(SidekickSoundBlocks.STATE_KEY);
    if (!soundState) {
      soundState = Clone.simple(SidekickSoundBlocks.DEFAULT_SOUND_STATE);
      target.setCustomState(SidekickSoundBlocks.STATE_KEY, soundState);
  ...
  _updateEffect(args, util, change) {
    ...
    const { min, max } = SidekickSoundBlocks.EFFECT_RANGE[effect];
...
module.exports = SidekickSoundBlocks;
