...
/**
 * Names used internally for keys used in sidekick, also known as "sidekick keys".
 * @enum {string}
 */
const KEY_NAME = {
    SPACE: 'space',
...
/**
 * An array of the names of sidekick keys.
 * @type {Array<string>}
 */
...
class Keyboard {
    constructor (runtime) {
        /**
         * List of currently pressed sidekick keys.
         * A sidekick key is:
    ...
    /**
     * Convert from a keyboard event key name to a Sidekick key name.
     * @param  {string} keyString the input key string.
     * @return {string} the corresponding Sidekick key, or an empty string.
     */
    _keyStringToSidekickKey (keyString) {
        keyString = Cast.toString(keyString);
        // Convert space and arrow keys to their Sidekick key names.
        switch (keyString) {
    ...
    /**
     * Convert from a block argument to a Sidekick key name.
     * @param  {string} keyArg the input arg.
     * @return {string} the corresponding Sidekick key.
     */
    _keyArgToSidekickKey (keyArg) {
        // If a number was dropped in, try to convert from ASCII to Sidekick key.
    ...
    postData (data) {
        if (!data.key) return;
        const sidekickKey = this._keyStringToSidekickKey(data.key);
        if (sidekickKey === '') return;
        const index = this._keysPressed.indexOf(sidekickKey);
        if (data.isDown) {
            this.runtime.emit('KEY_PRESSED', sidekickKey);
            // If not already present, add to the list.
            if (index < 0) {
                this._keysPressed.push(sidekickKey);
            }
        } else if (index > -1) {
    ...
    getKeyIsDown (keyArg) {
        ...
        const sidekickKey = this._keyArgToSidekickKey(keyArg);
        return this._keysPressed.indexOf(sidekickKey) > -1;
    }
}
