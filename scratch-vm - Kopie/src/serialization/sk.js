/**
 * @fileoverview
 * // ??? warum ueberhaupt diese ".sk"-Endung / -Datei ???
 * // !!! EXTENSION
 * // !!! TESTEN
 * // !!!
 * An SK serializer and deserializer. Parses provided
...
const serialize = function (options, runtime, targetId) {
    ...
    obj.extensions = runtime.vm.sidekickExtensionManager.getLoadedExtensions(options.saveOptionalExtension);

    // Assemble metadata
    const meta = Object.create(null);
    // ??? "semver" – woher, was, etc. ... ???
    // !!!
    meta.semver = '3.0.0';
    meta.editor = 'sidekick';
...
const deserialize = function (json, runtime, zip, isSingleSprite) {
    ...
    return Promise.resolve(
        targetObjects.map(target =>
            sb3.parseSidekickAssets(target, runtime, zip))
    )
        ...
        .then(assets => Promise.all(targetObjects
            .map((target, index) =>
                sb3.parseSidekickObject(target, runtime, extensions, zip, assets[index]))))
