 ...
 * JSON and then generates all needed sidekick-vm runtime structures.
 */
 ...
 * @property {Array.<Target>} targets - the imported Sidekick target objects.
...
/**
 * Parse the assets of a single "Sidekick object" and load them. This
 ...
 * @param {!object} object From-JSON "Sidekick object:" sprite, stage, watcher.
 ...
 */
const parseSidekickAssets = function (object, runtime, zip) {
...
const handleUnknownBlocks = function (blockJSON, extensionID, handleMethod, reporters) {
    ...
    alert('As the original code would conflict with Sidekick extension, this feature has been temporarily commented out. Might be updated in a future version of the Sidekick extension.');
    return;
    /*
    switch (handleMethod) {
    case 'replace': {
 ...
 * // ???
 * // !!!
 * Parse a single "Sidekick object" and create all its in-memory VM objects.
 * @param {!object} object From-JSON "Sidekick object:" sprite, stage, watcher.
 * @param {!Runtime} runtime Runtime object to load all structures into.
 * @param {ImportedExtensionsInfo} extensions - (in/out) parsed extension information will be stored here.
 * @param {JSZip} zip Sb3 file describing this project (to load assets from)
 * @param {object} assets - Promises for assets of this sidekick object grouped
 *   into costumes and sounds
 * @return {!Promise.<Target>} Promise for the target created (stage or sprite), or null for unsupported objects.
 */
const parseSidekickObject = function (object, runtime, extensions, zip, assets) {
    // Watcher/monitor - skip this object until those are implemented in VM.
    // @todo
    ...
    // ??? was ist dann "todo" ???
    // !!!
    // @todo: For now, load all Sidekick objects (stage/sprites) as a Sprite.
...
const deserialize = function (json, runtime, zip, isSingleSprite) {
    ...
    return Promise.resolve(
        targetObjects.map(target =>
            parseSidekickAssets(target, runtime, zip))
    )
        ...
        .then(assets => Promise.all(targetObjects
            .map((target, index) =>
                parseSidekickObject(target, runtime, extensions, zip, assets[index]))))
