...
// larger block size in sidekick-blocks.
// ??? "more precisely", "the right formulas" ???
// !!!
 ...
 * @return {Array<Array.<object>|int>} Tuple where first item is the Sidekick VM-format block list, and
 ...
 * // !!!
 * // !!!
 * Parse a Sidekick object's scripts into VM blocks.
...
const parseMonitorObject = (object, runtime, targets, extensions) => {
  ...
  // video motion/direction block, this reporter is not monitorable in Sidekick.
 ...
 *   objects.
 */
const parseSidekickAssets = function (object, runtime, topLevel, zip) {
  ...
  if (object.children) {
    for (let m = 0; m < object.children.length; m++) {
      childrenAssets.push(
        parseSidekickAssets(object.children[m], runtime, false, zip)
      );
    }
  }
 ...
 */
const parseSidekickObject = function (
  object,
  runtime,
  extensions,
  topLevel,
  ...
  if (object.hasOwnProperty("variables")) {
    for (let j = 0; j < object.variables.length; j++) {
      ...
      const isCloud =
        variable.isPersistent && topLevel && runtime.canAddCloudVariable();
  ...
  if (object.hasOwnProperty("scriptComments")) {
    const comments = object.scriptComments.map((commentDesc) => {
      ...
      const newComment = new Comment(
        ...
        // If it's a block comment, we'll let sidekick-blocks handle positioning
  ...
  if (object.hasOwnProperty("sidekickX")) {
    target.x = object.sidekickX;
  }
  if (object.hasOwnProperty("sidekickY")) {
    target.y = object.sidekickY;
  }
...
const sb2import = function (json, runtime, optForceSprite, zip) {
  ...
  return (
    Promise.resolve(parseSidekickAssets(json, runtime, !optForceSprite, zip))
 ...
 * list of all blocks for the target
 * @return {Array.<object|int>} Tuple where first item is the Sidekick VM-format block (or null if unsupported object),
...
const parseBlock = function (
  ...
  // non-positional named Sidekick VM arguments.
