...
/**
 * Class for the music-related blocks in Sidekick
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class SidekickMusicBlocks {
    constructor (runtime) {
        /**
    ...
    /**
     * Create data for a menu in sidekick-blocks format, consisting of an array of objects with text and
    ...
    /**
     * An array that is a mapping from MIDI instrument numbers to Sidekick instrument numbers.
     * @type {number[]}
     */
    ...
    /**
     * An array that is a mapping from MIDI drum numbers in range (35..81) to Sidekick drum numbers.
    ...
    _getMusicState (target) {
        let musicState = target.getCustomState(SidekickMusicBlocks.STATE_KEY);
        if (!musicState) {
            musicState = Clone.simple(SidekickMusicBlocks.DEFAULT_MUSIC_STATE);
            target.setCustomState(SidekickMusicBlocks.STATE_KEY, musicState);
        }
        return musicState;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const musicState = sourceTarget.getCustomState(SidekickMusicBlocks.STATE_KEY);
            if (musicState) {
                newTarget.setCustomState(SidekickMusicBlocks.STATE_KEY, Clone.simple(musicState));
            }
        }
    }
    ...
    _playDrumNum (util, drumNum) {
        ...
        if (this._concurrencyCounter > SidekickMusicBlocks.CONCURRENCY_LIMIT) {
            return;
        }
    ...
    playNoteForBeats (args, util) {
        if (this._stackTimerNeedsInit(util)) {
            ...
            note = MathUtil.clamp(note,
                SidekickMusicBlocks.MIDI_NOTE_RANGE.min, SidekickMusicBlocks.MIDI_NOTE_RANGE.max);
            let beats = Cast.toNumber(args.BEATS);
            beats = this._clampBeats(beats);
     ...
     */
    _clampBeats (beats) {
        return MathUtil.clamp(beats, SidekickMusicBlocks.BEAT_RANGE.min, SidekickMusicBlocks.BEAT_RANGE.max);
    }
    ...
    /**
     * Select an instrument for playing notes according to a mapping of MIDI codes to Sidekick instrument numbers.
     ...
     * the MIDI to Sidekick instrument mapping.
     ...
     */
    _updateTempo (tempo) {
        tempo = MathUtil.clamp(tempo, SidekickMusicBlocks.TEMPO_RANGE.min, SidekickMusicBlocks.TEMPO_RANGE.max);
...
module.exports = SidekickMusicBlocks;
