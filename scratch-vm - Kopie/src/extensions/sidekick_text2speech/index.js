 ...
 * // ???
 * // !!!
 * Language ids. The value for each language id is a valid Sidekick locale.
 */
const ARABIC_ID = 'ar';
 ...
 * @constructor
 */
class SidekickText2SpeechBlocks {
    constructor (runtime) {
        /**
         ...
         * // ???
         * // !!!
         * A list of all Sidekick locales that are supported by the extension.
         * @type {Array}
         */
     ...
     * SUPPORTED LOCALE
     *      A Sidekick locale that has a corresponding extension locale.
     * EXTENSION LOCALE
     ...
     */
    _getState (target) {
        let state = target.getCustomState(SidekickText2SpeechBlocks.STATE_KEY);
        if (!state) {
            state = Clone.simple(SidekickText2SpeechBlocks.DEFAULT_TEXT2SPEECH_STATE);
            target.setCustomState(SidekickText2SpeechBlocks.STATE_KEY, state);
        }
        return state;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const state = sourceTarget.getCustomState(SidekickText2SpeechBlocks.STATE_KEY);
            if (state) {
                newTarget.setCustomState(SidekickText2SpeechBlocks.STATE_KEY, Clone.simple(state));
            }
        }
    }
     ...
     * browser locale.
     * @return {string} a Sidekick locale code.
     */
    getEditorLanguage () {
     ...
     * @returns {string} a Sidekick locale code.
     */
    getCurrentLanguage () {
    ...
    /**
     * Check if a Sidekick language code is in the list of supported languages for the
...
module.exports = SidekickText2SpeechBlocks;
