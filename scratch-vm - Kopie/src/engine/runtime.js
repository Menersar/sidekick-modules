...
const SidekickBlocksConstants = require("./sidekick-blocks-constants");
...
const SidekickLinkWebSocket = require("../util/sidekick-link-websocket");
...
const defaultBlockPackages = {
  sidekick_control: require("../blocks/sidekick_control"),
  sidekick_event: require("../blocks/sidekick_event"),
  sidekick_looks: require("../blocks/sidekick_looks"),
  sidekick_motion: require("../blocks/sidekick_motion"),
  sidekick_operators: require("../blocks/sidekick_operators"),
  sidekick_sound: require("../blocks/sidekick_sound"),
  sidekick_sensing: require("../blocks/sidekick_sensing"),
  sidekick_data: require("../blocks/sidekick_data"),
  sidekick_procedures: require("../blocks/sidekick_procedures"),
};

const defaultExtensionColors = ["#b7d698", "#a4bf88", "#90a877"];

/**
 * Information used for converting Sidekick argument types into sidekick-blocks data.
...
const ArgumentTypeMap = (() => {
  ...
  map[ArgumentType.ANGLE] = {
    shadow: {
      ...
      // the sidekick-blocks defaults for that field will be
...
class Runtime extends EventEmitter {
  ...
  constructor(vm) {
    ...
    /**
     * The version of Sidekick.
     * It should be set by sidekick-gui.
     * @type {String}
     */
    ...
    /**
     * // ??? "origin of the current project", "CSFirst" ???
     * // !!!
     * // !!!
  ...
  _fillExtensionCategory(categoryInfo, extensionInfo) {
    ...
    for (const menuName in extensionInfo.menus) {
      if (extensionInfo.menus.hasOwnProperty(menuName)) {
        ...
        const convertedMenu = this._buildMenuForSidekickBlocks(
          menuName,
          menuInfo,
          categoryInfo
        );
    ...
    for (const blockInfo of extensionInfo.blocks) {
      try {
        const convertedBlock = this._convertForSidekickBlocks(
          blockInfo,
          categoryInfo
        );
   ...
   * // ???
   * // !!!
   * Convert the given extension menu items into the sidekick-blocks style of list of pairs.
   ...
   * // !!!
   * // !!!
   * Build the sidekick-blocks JSON for a menu. Note that sidekick-blocks treats menus as a special kind of block.
   * @param {string} menuName - the name of the menu
   ...
   * @returns {object} - a JSON-esque object ready for sidekick-blocks' consumption
   * @private
   */
  _buildMenuForSidekickBlocks(menuName, menuInfo, categoryInfo) {
    ...
    return {
      json: {
        ...
        outputShape: menuInfo.acceptReporters
          ? SidekickBlocksConstants.OUTPUT_SHAPE_ROUND
          : SidekickBlocksConstants.OUTPUT_SHAPE_SQUARE,
        args0: [
          {
            type: "field_dropdown",
            name: menuName,
  ...
  _buildCustomFieldInfo(fieldName, fieldInfo, extensionId, categoryInfo) {
    ...
    return {
      ...
      sidekickBlocksDefinition: this._buildCustomFieldTypeForSidekickBlocks(
        extendedName,
        fieldInfo.output,
  ...
  /**
   * Build the sidekick-blocks JSON needed for a fieldType.
   ...
   * // ???
   * // !!!
   * @param {number} outputShape - Shape of the field (from SidekickBlocksConstants)
   * @param {object} categoryInfo - The category the field belongs to (Used to set its colors)
   * @returns {object} - Object to be inserted into sidekick-blocks
   */
  _buildCustomFieldTypeForSidekickBlocks(
    fieldName,
    output,
    outputShape,
    categoryInfo
  ...
  /**
   * Convert ExtensionBlockMetadata into data ready for sidekick-blocks.
   * @param {ExtensionBlockMetadata} blockInfo - the block info to convert
   * @param {CategoryInfo} categoryInfo - the category for this block
   * @returns {ConvertedBlockInfo} - the converted & original block information
   * @private
   */
  _convertForSidekickBlocks(blockInfo, categoryInfo) {
    if (blockInfo === "---") {
      return this._convertSeparatorForSidekickBlocks(blockInfo);
    }

    if (blockInfo.blockType === BlockType.BUTTON) {
      return this._convertButtonForSidekickBlocks(blockInfo);
    }

    return this._convertBlockForSidekickBlocks(blockInfo, categoryInfo);
  }

  /**
   * Convert ExtensionBlockMetadata into sidekick-blocks JSON & XML, and generate a proxy function.
   * @param {ExtensionBlockMetadata} blockInfo - the block to convert
   * @param {CategoryInfo} categoryInfo - the category for this block
   * @returns {ConvertedBlockInfo} - the converted & original block information
   * @private
   */
  _convertBlockForSidekickBlocks(blockInfo, categoryInfo) {
    ...
    switch (blockInfo.blockType) {
      case BlockType.COMMAND:
        blockJSON.outputShape = SidekickBlocksConstants.OUTPUT_SHAPE_SQUARE;
      ...
      case BlockType.REPORTER:
        ...
        blockJSON.outputShape = SidekickBlocksConstants.OUTPUT_SHAPE_ROUND;
        break;
      case BlockType.BOOLEAN:
        blockJSON.output = "Boolean";
        blockJSON.outputShape = SidekickBlocksConstants.OUTPUT_SHAPE_HEXAGONAL;
        break;
      case BlockType.HAT:
    ...
    // ???
    // !!!
    let outLineNum = 0; // used for sidekick-blocks `message${outLineNum}` and `args${outLineNum}`
      ...
      blockJSON[`args${outLineNum}`] = [
        {
          ...
          // ??? "*", "collapsed blocks" ???
          // !!!
          alt: "*", // TODO remove this since we don't use collapsed blocks in sidekick
          flip_rtl: true,
        },
      ];
      ++outLineNum;
   ...
   * @private
   */
  _convertSeparatorForSidekickBlocks(blockInfo) {
    return {
      info: blockInfo,
      xml: '<sep gap="36"/>',
    };
   ...
   * // !!!
   * // !!!
   * Convert a button for sidekick-blocks. A button has no opcode but specifies a callback name in the `func` field.
   ...
   * @private
   */
  _convertButtonForSidekickBlocks(buttonInfo) {
    // ??? "pre-defined callbacks",  "pre-defined callbacks handled in sidekick-blocks" ???
    // !!!
    // for now we only support these pre-defined callbacks handled in sidekick-blocks
   ...
   * // !!!
   * @return {object} JSON blob for a sidekick-blocks image field.
   * @private
   */
  ...
  /**
   * Helper for _convertForSidekickBlocks which handles linearization of argument placeholders. Called as a callback
   * from string#replace. In addition to the return value the JSON and XML items in the context will be filled.
   * @param {object} context - information shared with _convertForSidekickBlocks about the block, etc.
   ...
   * // ??? "sidekick-blocks placeholder for the argument", "'%1'", warum %1, Bedeutung %1 ???
   * // !!!
   * @return {string} sidekick-blocks placeholder for the argument: '%1'.
   * @private
   */
  _convertPlaceholders(context, match, placeholder) {
    ...
    // !!!
    // !!!
    // Start to construct the sidekick-blocks style JSON defining how the block should be
    // laid out
    let argJSON;
    ...
    if (argTypeInfo.fieldType === "field_image") {
      ...
      if (argInfo.menu) {
        ...
        if (argInfo.shadow === undefined || argInfo.shadow === true) {
          ...
          fieldName =
            (argTypeInfo.shadow && argTypeInfo.shadow.fieldName) || null;
        }
      }

      // <value> is the SidekickBlocks name for a block input.
      if (valueName) {
  ...
  /**
   * @returns {Array.<object>} sidekicks-blocks XML for each category of extension blocks, in category order.
  ...
  /**
   * @returns {Array.<string>} - an array containing the sidekick-blocks JSON information for each dynamic block.
   */
  getBlocksJSON() {
    return this._blockInfo.reduce(
      (result, categoryInfo) =>
        result.concat(categoryInfo.blocks.map((blockInfo) => blockInfo.json)),
      []
    );
  }

  /**
   * One-time initialization for Sidekick Link support.
   */
  _initSidekickLink() {
    /* global globalThis */
    ...
    if (globalThis.document && document.getElementById) {
      ...
      // !!! TESTEN !!!
      // !!!
      // Create a script tag for the Sidekick Link browser extension, unless one already exists
      const scriptElement = document.getElementById(
        "sidekick-link-extension-script"
      );
      if (!scriptElement) {
        const script = document.createElement("script");
        script.id = "sidekick-link-extension-script";
        document.body.appendChild(script);

        // Tell the browser extension to inject its script.
        // If the extension isn't present or isn't active, this will do nothing.
        globalThis.postMessage("inject-sidekick-link-script", globalThis.origin);
      }
    }
  }

  /**
   * Get a sidekick link socket.
   * @param {string} type Either BLE or BT
   * // ???
   * // !!!
   * @returns {SidekickLinkSocket} The sidekick link socket.
   */
  getSidekickLinkSocket(type) {
    // ??? "_defaultSidekickLinkSocketFactory" – wo, woher, etc. ... ???
    // !!!
    const factory =
      this._linkSocketFactory || this._defaultSidekickLinkSocketFactory;
    return factory(type);
  }
   ...
   * // ??? "BT", "BLE" ???
   * // !!!
   * Configure how SidekickLink sockets are created. Factory must consume a "type" parameter
   * either BT or BLE.
   * // ??? "new factory" ???
   * @param {Function} factory The new factory for creating SidekickLink sockets.
   */
  configureSidekickLinkSocketFactory(factory) {
    this._linkSocketFactory = factory;
  }

  /**
   * The default sidekick link socket creator, using websockets to the installed device manager.
   * @param {string} type Either BLE or BT
   * @returns {SidekickLinkSocket} The new sidekick link socket (a WebSocket object)
   */
  _defaultSidekickLinkSocketFactory(type) {
    // ??? "self.Sidekick" ???
    // !!!
    const Sidekick = self.Sidekick;
    // ??? "SidekickLinkSafariSocket" ???
    // !!!
    const SidekickLinkSafariSocket =
      Sidekick && Sidekick.SidekickLinkSafariSocket;
    // detect this every time in case the user turns on the extension after loading the page
    const useSafariSocket =
      SidekickLinkSafariSocket &&
      SidekickLinkSafariSocket.isSafariHelperCompatible();
    return useSafariSocket
      ? new SidekickLinkSafariSocket(type)
      : new SidekickLinkWebSocket(type);
  }
  ...
  getIsEdgeActivatedHat(opcode) {
    return (
      this._hats.hasOwnProperty(opcode) && this._hats[opcode].edgeActivated
    );
  }
  ...
  /**
   * // ??? schon irgendwo anders gesehen – eventuell doppelt / obsolete ???
   ...
   * @param {!SidekickStorage} storage The storage module to attach
   */
  ...
  startHats(requestedHatOpcode, optMatchFields, optTarget) {
    ...
    // threads are stepped. See SidekickRuntime.as for original implementation
  ...
  dispose() {
    ...
    this.renderer._allSkins.forEach((skin) => {
      if (skin && skin.constructor.name !== "PenSkin")
   ...
   * // ??? "Target" ???
   * // !!!
   * Get a target representing the Sidekick stage, if one exists.
   * @return {?Target} The target, if found.
   */
  getTargetForStage() {
    for (let i = 0; i < this.targets.length; i++) {
  ...
  registerExtension(extensionId) {
    if (this.extensions.includes(extensionId)) {
      throw new Error(
        `Failed to register an existed extension: ${extensionId}`
      );
    }
  ...
  unregisterExtension(extensionId) {
    ...
    if (index == -1) {
      throw new Error(
        `Failed to unregister an unexisted extension: ${extensionId}`
      );
    }
  ...
  getProcedureParamNamesIdsAndDefaults(name) {
    for (const target of this.targets) {
      // TODO: check if it is global
      const definition =
        target.blocks.getProcedureParamNamesIdsAndDefaults(name);
      if (definition) {
        return definition;
      }
    }
