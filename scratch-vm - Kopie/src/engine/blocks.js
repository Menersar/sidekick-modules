 ...
 * and handle updates from Sidekick Blocks events.
 */
...
class Blocks {
  ...
  blocklyListen(e) {
    ...
    switch (e.type) {
      ...
      case "var_create":
        ...
        if (
          ...
          for (const target of allTargets) {
            if (
              target.lookupVariableByNameAndType(e.varName, e.varType, true)
            ) {
              return;
            }
          }
      ...
      case "var_delete": {
        const target =
          editingTarget && editingTarget.variables.hasOwnProperty(e.varId)
            ? editingTarget
            : stage;
      ...
      case "comment_create":
        if (this.runtime.getEditingTarget()) {
          ...
          if (
            ...
            // x and y coordinates set to null so that sidekick-blocks can
      ...
      case "func_modify": {
        ...
        if (oldMutation.global === "true") {
          ...
          for (let i = 0; i < targets.length; i++) {
            const currTarget = targets[i];
            currTarget.blocks.updateBlocksAfterFuncModify(
              proccode,
              newMutation,
              currTarget === editingTarget
            );
          }
        } else {
          // This is a local function originally
          editingTarget.blocks.updateBlocksAfterFuncModify(
            proccode,
            newMutation,
            true
          );
    ...
    switch (args.element) {
      ...
      case "checkbox": {
        ...
        if (block.opcode === "data_variable") {
          ...
          isSpriteLocalVariable =
            !this.runtime.getTargetForStage().variables[block.fields.LIST.id];
        }
  ...
  updateBlocksAfterFuncModify(proccode, newMutation, isEditingTarget) {
    ...
    for (const blockId in blocks) {
      ...
      if (
        ...
        if (block.mutation.return !== newMutation.return) {
          block.opcode = `procedures_call${
            newMutation.return === "true" ? "_return" : ""
          }`;
        }
   ...
   * by a Blockly/sidekick-blocks workspace.
   ...
   * into a Blockly/sidekick-blocks XML string.
