...
if (window.performance) {
    ...
    performance.mark('Sidekick.EvalStart');
}
...
const SidekickStorage = require('scratch-storage');
const VirtualMachine = require('..');
const Runtime = require('../engine/runtime');

const SidekickRender = require('scratch-render');
const AudioEngine = require('scratch-audio');
const SidekickSVGRenderer = require('scratch-svg-renderer');

const Sidekick = window.Sidekick = window.Sidekick || {};
...
const loadProject = function () {
    ...
    Sidekick.vm.downloadProjectId(id);
    return id;
};
    ...
    on (storage, vm) {
        ...
        storage.webHelper.load = function (...args) {
            if (_this.dataLoaded === 0 && window.performance) {
                ...
                performance.mark('Sidekick.LoadDataStart');
            }
            ...
            if (_this.dataLoaded) {
                if (_this.contentTotal === 0 && window.performance) {
                    performance.mark('Sidekick.DownloadStart');
                }
            ...
            result.then(() => {
                if (_this.dataLoaded === 0) {
                    if (window.performance) {
                        ...
                        performance.mark('Sidekick.LoadDataEnd');
                        performance.measure('Sidekick.LoadData', 'Sidekick.LoadDataStart', 'Sidekick.LoadDataEnd');
                    }

                    _this.dataLoaded = 1;

                    window.SidekickVMLoadDataEnd = Date.now();
                } else {
                ...
                if (_this.contentComplete && _this.contentComplete === _this.contentTotal) {
                    if (window.performance) {
                        ...
                        performance.mark('Sidekick.DownloadEnd');
                        performance.measure('Sidekick.Download', 'Sidekick.DownloadStart', 'Sidekick.DownloadEnd');
                    }

                    window.SidekickVMDownloadEnd = Date.now();
                }
        ...
        vm.runtime.on(Runtime.PROJECT_LOADED, () => {
            ...
            if (window.performance) {
                ...
                // all the project assets?
                performance.mark('Sidekick.LoadEnd');
                performance.measure('Sidekick.Load', 'Sidekick.LoadStart', 'Sidekick.LoadEnd');
            }

            window.SidekickVMLoadEnd = Date.now();
...
const runBenchmark = function () {
    ...
    const storage = new SidekickStorage();
    ...
    const renderer = new SidekickRender(canvas);
    Sidekick.renderer = renderer;
    vm.attachRenderer(renderer);
    const audioEngine = new AudioEngine();
    vm.attachAudioEngine(audioEngine);
    vm.attachV2BitmapAdapter(new SidekickSVGRenderer.BitmapAdapter());
    ...
    document.addEventListener('mousemove', e => {
        ...
        Sidekick.vm.postIOData('mouse', coordinates);
    });
    canvas.addEventListener('mousedown', e => {
        ...
        Sidekick.vm.postIOData('mouse', data);
        e.preventDefault();
    });
    ...
    document.addEventListener('keydown', e => {
        ...
        Sidekick.vm.postIOData('keyboard', {
            keyCode: e.keyCode,
            isDown: true
        });
        e.preventDefault();
...
if (window.performance) {
    performance.mark('Sidekick.EvalEnd');
    performance.measure('Sidekick.Eval', 'Sidekick.EvalStart', 'Sidekick.EvalEnd');
}

window.SidekickVMEvalEnd = Date.now();
