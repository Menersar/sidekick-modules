...
const SidekickBlocksConstants = require("../engine/sidekick-blocks-constants");
...
const ArgumentTypeMap = (() => {
  ...
  map[ArgumentType.ANGLE] = {
    shadow: {
      ...
      // ??? "default of 0 for number fields" – kann es da potenziell Probleme geben ???
      // !!!
...
class ExtensionAPI {
  ...
  // Search the _blockInfo array of this.vm.runtime for the given category;
  // return "true" as soon as it is found;
  // return "false" if it could not be found.
  _doesCategoryExist(categoryId) {
    for (const category of this.vm.runtime._blockInfo) {
      if (category.id === categoryId) {
        return true;
      }
    ...
    return new Promise((resolve, reject) => {
      for (const i in this.categoryInfo) {
        ...
        for (const j in this.categoryInfo[i].blocks) {
          ...
          let block = this.blocks.get(
            this.categoryInfo[i].blocks[j].info.opcode
          );
          for (const paramId in block.param) {
            // if the param doesn't have menu or it's an field
            if (!block.param[paramId].menu || block.param[paramId].field)
              continue;
            ...
            this.categoryInfo[i].menus.push({
              json: {
                ...
                outputShape: SidekickBlocksConstants.OUTPUT_SHAPE_ROUND,
                args0: [
                  {
                    type: "field_dropdown",
                    name: paramId,
          ...
          this._processBlockArguments(
            context,
            this.blocks.get(this.categoryInfo[i].blocks[j].info.opcode)
          );
        }
  ...
  addCategory(category) {
    ...
    // !!!
    // !!! TESTEN
    // !!! Muesste nicht eig genau das Gegenteil hier passieren?
    // Prevent adding the same category multiple times
    // if (!!this._getCategory(category.categoryId)) {
    // Check via _doesCategoryExist() and with the category's id if it has already been added.
    if (this._doesCategoryExist(category.categoryId)) {
      console.warn("Can not add the same category twice.");
      return;
    }
  ...
  removeCategory(categoryId) {
    // Prevent attempt to uninstall / remove a non-existent category
    if (!this._getCategory(categoryId)) {
      console.warn("Can not remove a non-existent category.");
      return;
    }
