...
/**
 * Host for the Pen-related blocks in Sidekick
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class SidekickPenBlocks {
    constructor (runtime) {
        /**
    ...
    _clampPenSize (requestedSize) {
        return MathUtil.clamp(
            requestedSize,
            SidekickPenBlocks.PEN_SIZE_RANGE.min,
            SidekickPenBlocks.PEN_SIZE_RANGE.max
        );
    }
     ...
     */
    _getPenState (target) {
        let penState = target.getCustomState(SidekickPenBlocks.STATE_KEY);
        if (!penState) {
            penState = Clone.simple(SidekickPenBlocks.DEFAULT_PEN_STATE);
            target.setCustomState(SidekickPenBlocks.STATE_KEY, penState);
        }
        return penState;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const penState = sourceTarget.getCustomState(SidekickPenBlocks.STATE_KEY);
            if (penState) {
                newTarget.setCustomState(SidekickPenBlocks.STATE_KEY, Clone.simple(penState));
                if (penState.penDown) {
...
module.exports = SidekickPenBlocks;
