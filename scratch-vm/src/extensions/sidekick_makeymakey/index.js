...
/**
 * Names used by keyboard io for keys used in sidekick.
 * @enum {string}
 */
const SIDEKICK_KEY_NAME = {
    [KEY_ID_SPACE]: 'space',
...
/**
 * Class for the makey makey blocks in Sidekick
 * @constructor
 */
class SidekickMakeyMakeyBlocks {
    constructor (runtime) {
        /**
    ...
    */
    ...
    whenMakeyKeyPressed (args, util) {
        ...
        if (SIDEKICK_KEY_NAME[args.KEY]) {
            key = SIDEKICK_KEY_NAME[args.KEY];
        }
     ...
     * // !!!
     * // ???
     * @param {string} key A sidekick key name.
     */
    keyPressed (key) {
        // ???
        // !!!
        // Store only the first word of the Sidekick key name, so that e.g. when
...
module.exports = SidekickMakeyMakeyBlocks;
