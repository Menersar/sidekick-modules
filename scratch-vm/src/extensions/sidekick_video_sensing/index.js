...
// !!
// !! 
// !! Cannot redeclare block-scoped variable 'VideoState'.
...
/**
 * Class for the motion-related blocks in Sidekick
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class SidekickVideoSensingBlocks {
    constructor (runtime) {
        /**
    ...
    reset () {
        ...
        for (let i = 0; i < targets.length; i++) {
            const state = targets[i].getCustomState(SidekickVideoSensingBlocks.STATE_KEY);
            if (state) {
     ...
     */
    _loop () {
        const loopTime = Math.max(this.runtime.currentStepTime, SidekickVideoSensingBlocks.INTERVAL);
        ...
        if (offset > SidekickVideoSensingBlocks.INTERVAL) {
            const frame = this.runtime.ioDevices.video.getFrame({
                format: Video.FORMAT_IMAGE_DATA,
                dimensions: SidekickVideoSensingBlocks.DIMENSIONS
            });
            if (frame) {
                this._lastUpdate = time;
    ...
    /**
     * Create data for a menu in sidekick-blocks format, consisting of an array
    ...
    _getMotionState (target) {
        let motionState = target.getCustomState(SidekickVideoSensingBlocks.STATE_KEY);
        if (!motionState) {
            motionState = Clone.simple(SidekickVideoSensingBlocks.DEFAULT_MOTION_STATE);
            target.setCustomState(SidekickVideoSensingBlocks.STATE_KEY, motionState);
        }
        return motionState;
    }
    ...
    /**
     * A sidekick reporter block handle that analyzes the last two frames and
    ...
    /**
     * A sidekick hat block edge handle that analyzes the last two frames where
    ...
    /**
     * A sidekick command block handle that configures the video state from
     * passed arguments.
     * @param {object} args - the block arguments
    ...
    /**
     * A sidekick command block handle that configures the video preview's
...
module.exports = SidekickVideoSensingBlocks;
